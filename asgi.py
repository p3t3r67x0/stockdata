...
async def insert_dataframes(d, s):
    a = yf.Ticker(s)

    for i in d.iterrows():
        try:
            await col.insert_one({'timestamp': i[0], 'open': i[1][0],
...
async def list_all_symbols():
    ...
    symbols = await read_symbols(col)
...
async def average_values(symbol):
    ...
    high_one_day = await read_average(col, symbol, 'high', 1)
    object['high_one_day'] = high_one_day

    high_ten_days = await read_average(col, symbol, 'high', 10)
    object['high_ten_days'] = high_ten_days

    high_thirty_days = await read_average(col, symbol, 'high', 30)
    object['high_thirty_days'] = high_thirty_days

    low_one_day = await read_average(col, symbol, 'low', 1)
    object['low_one_day'] = low_one_day

    low_ten_days = await read_average(col, symbol, 'low', 10)
    object['low_ten_days'] = low_ten_days

    low_thirty_days = await read_average(col, symbol, 'low', 30)
    object['low_thirty_days'] = low_thirty_days

    open_one_day = await read_average(col, symbol, 'open', 1)
    object['open_one_day'] = open_one_day

    open_ten_days = await read_average(col, symbol, 'open', 10)
    object['open_ten_days'] = open_ten_days

    open_thirty_days = await read_average(col, symbol, 'open', 30)
    object['open_thirty_days'] = open_thirty_days

    close_one_day = await read_average(col, symbol, 'close', 1)
    object['close_one_day'] = close_one_day

    close_ten_days = await read_average(col, symbol, 'close', 10)
    object['close_ten_days'] = close_ten_days

    close_thirty_days = await read_average(col, symbol, 'close', 30)
    object['close_thirty_days'] = close_thirty_days

    adjust_one_day = await read_average(col, symbol, 'adjust_close', 1)
    object['adjust_one_day'] = adjust_one_day

    adjust_ten_days = await read_average(col, symbol, 'adjust_close', 10)
    object['adjust_ten_days'] = adjust_ten_days

    adjust_thirty_days = await read_average(
        col, symbol, 'adjust_close', 30)
